...
export interface LatestNewsCardProps {
  ...
  newsItems: NewsItem[];
  /** Optional click handler for the "View All" link */
  onViewAllClick?: () => void;
}
...
export const LatestNewsCard: React.FC<LatestNewsCardProps> = ({
  ...
  return (
    <div className="w-full max-w-3xl rounded-xl border bg-card p-6 text-card-foreground shadow-sm">
      {/* Card Header */}
      <div className="mb-4 flex items-center justify-between gap-4">
        <h2 className="text-base sm:text-lg font-semibold">{title}</h2>
        <a
          href={viewAllHref}
          className="text-xs sm:text-sm font-medium text-primary transition-colors hover:text-primary/80"
          ...
          <motion.li key={item.id} variants={itemVariants}>
            ...
            >
              <img
                ...
                className="h-14 w-14 sm:h-16 sm:w-16 flex-shrink-0 rounded-md object-cover"
              />
              <div className="flex-grow">
                <h3 className="text-sm sm:text-base font-medium leading-tight text-card-foreground line-clamp-2">
                  {item.title}
                </h3>
